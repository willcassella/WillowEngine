...
void InitRenderer(uint32 width, uint32 height)
{
	...
	// Create a framebuffer for deferred rendering
	...
	// Create a diffuse buffer for the FBO
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL); // 4 8-bit unsigned integer components for color
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, diffuseBuffer, 0);

	// Create a normal buffer for the FBO
	...
	// Create a specular buffer for the FBO
	...
	// Create a metallic buffer for the FBO
	...
	// Create a roughness buffer for the FBO
	...
	// Set OpenGL to draw to the color buffer, normal buffer, and specular buffer
	...
	// Make sure the framebuffer was constructed successfully
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE)
	{
		Console::WriteLine("Framebuffer created successfully");
	}
	else
	{
		Console::WriteLine("Error creating the framebuffer");
	}
}
